...
/* 탑바 */
.topbar{display:flex;align-items:center;justify-content:space-between;padding:0 1.5rem;height:54px;background:#fff;border-bottom:1px solid #dee2e6;box-shadow:0 1px 4px rgba(0,0,0,.06);position:sticky;top:0;z-index:100}
.topbar h1{font-size:15px;font-weight:700;color:#212529}
.topbar h1 span{color:#3b82f6}

/* 사이드바 */
.sidebar-panel{width:260px;flex-shrink:0;background:#fff;border-right:1px solid #dee2e6;height:calc(100vh - 54px);overflow-y:auto}
.sel-wrap{padding:10px 14px;border-bottom:1px solid #dee2e6}
.sel-label{font-size:10px;font-weight:600;color:#868e96;letter-spacing:.8px;text-transform:uppercase;display:block;margin-bottom:5px}
.stats-row{display:grid;grid-template-columns:1fr 1fr;gap:6px;padding:10px 14px;border-bottom:1px solid #dee2e6}
.stat{background:#f8f9fa;border-radius:8px;padding:10px 12px;border:1px solid #dee2e6}
.stat .n{font-size:18px;font-weight:700;color:#3b82f6;line-height:1}
.stat .l{font-size:10px;color:#868e96;margin-top:3px}
.list-hdr{display:flex;justify-content:space-between;padding:7px 14px;background:#f1f3f5;border-bottom:1px solid #dee2e6}
.list-hdr span{font-size:10px;font-weight:600;color:#868e96;text-transform:uppercase;letter-spacing:.5px}
.list-item{display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid #f1f3f5}
.item-name{font-size:12px;font-weight:600;color:#212529}
.item-sub{font-size:10px;color:#868e96;margin-top:2px}
.item-emp{font-size:13px;font-weight:700;color:#10b981}
.pill-c{background:#dbeafe;color:#1d4ed8;padding:1px 6px;border-radius:3px;font-size:9px;font-weight:600}
.pill-i{background:#d1fae5;color:#065f46;padding:1px 6px;border-radius:3px;font-size:9px;font-weight:600}

/* Streamlit 요소 커스텀 */
[data-testid="stSelectbox"] > div > div{border:1px solid #dee2e6!important;border-radius:6px!important;font-size:12px!important}
[data-testid="stFileUploader"]{border:none!important}
div.stButton > button{background:#3b82f6!important;color:#fff!important;border:none!important;border-radius:6px!important;font-size:12px!important;font-weight:600!important;padding:8px 16px!important;width:100%!important}
div.stButton > button:hover{background:#2563eb!important}
[data-testid="stTextInput"] input{border:1px solid #dee2e6!important;border-radius:6px!important;font-size:12px!important;padding:7px 10px!important}
</style>
...
st.markdown("""
<div class="topbar">
  <h1>국민연금 <span>가입 사업장</span> 내역</h1>
</div>
""", unsafe_allow_html=True)
...
with col_side:
    # 파일 업로드
    st.markdown('<div class="sel-wrap"><span class="sel-label">📂 파일 업로드</span></div>', unsafe_allow_html=True)
    ...
    if uploaded:
        ...
        # 시도 선택
        st.markdown('<div class="sel-wrap"><span class="sel-label">시 / 도</span></div>', unsafe_allow_html=True)
        ...
        # 구/군 선택
        st.markdown('<div class="sel-wrap"><span class="sel-label">구 / 군</span></div>', unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="stats-row">
          <div class="stat"><div class="n">{total:,}</div><div class="l">전체</div></div>
          <div class="stat"><div class="n">{corp:,}</div><div class="l">법인</div></div>
          <div class="stat"><div class="n">{emp:,}</div><div class="l">총 직원</div></div>
          <div class="stat"><div class="n">{avg_e:,}</div><div class="l">평균 직원</div></div>
        </div>
        """, unsafe_allow_html=True)

        # 검색
        st.markdown('<div class="sel-wrap"><span class="sel-label">사업장 검색</span></div>', unsafe_allow_html=True)
        ...
        # 리스트
        st.markdown('<div class="list-hdr"><span>사업장</span><span>직원수</span></div>', unsafe_allow_html=True)
