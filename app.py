...
st.set_page_config(page_title="사업장 보완", page_icon="📋", layout="centered")
st.markdown("""<style>
...
# ── UI ──────────────────────────────────────────────────
st.title("📋 사업장 주소·전번 보완")
st.caption("대시보드에서 다운로드한 **지역 파일**을 업로드하세요 (전국 파일 ❌)")
st.divider()
...
if uploaded:
    ...
    if st.button("🚀 주소·전번 보완 시작", type="primary"):
        ...
        st.success(f"✅ 완료! 📞 {phone_ok:,}개 | 📍 {coord_ok:,}개 | 최종 {after:,}개 (삭제 {before-after:,}개)")
        st.balloons()
